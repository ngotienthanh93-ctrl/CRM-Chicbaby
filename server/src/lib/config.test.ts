...
describe('KV-02 · isValidKiotVietUrl (allowlist chống SSRF/exfil)', () => {
  it('chấp nhận https + miền kiotviet.vn (kể cả subdomain)', () => {
    expect(isValidKiotVietUrl('https://public.kiotviet.vn')).toBe(true);
    ...
    // default trong catalogue phải hợp lệ
  ...
  it('từ chối http, host lạ, userinfo, host giả mạo, rỗng', () => {
    expect(isValidKiotVietUrl('http://public.kiotviet.vn')).toBe(false); // không https
