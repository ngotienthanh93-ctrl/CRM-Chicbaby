...
/** Tập mã "đang mở" mặc định (đọc từ cấu hình catalogue) — dùng cho test isOpenOrderStatus. */
const DEFAULT_OPEN = parseOpenOrderStatuses(DEFAULT_ENGINE_CONFIG.sync.openOrderStatuses);
...
describe('isOpenOrderStatus — trạng thái đơn KiotViet theo tập cấu hình', () => {
  it('mã/chữ đang mở => true (phiếu tạm / đang giao)', () => {
    for (const s of ['1', '2', 'draft', 'Processing', 'PENDING', ' dang_giao ']) {
      expect(isOpenOrderStatus(s, DEFAULT_OPEN)).toBe(true);
    }
  });

  it('terminal/không rõ => false (hoàn thành/hủy/null/rỗng)', () => {
    for (const s of ['3', '4', 'completed', 'voided', 'xong', '', null, undefined]) {
      expect(isOpenOrderStatus(s, DEFAULT_OPEN)).toBe(false);
    }
  });

  it('cấu hình được: chỉ tập truyền vào mới coi là mở', () => {
    const custom = parseOpenOrderStatuses('open, in_transit');
    expect(isOpenOrderStatus('open', custom)).toBe(true);
    expect(isOpenOrderStatus('IN_TRANSIT', custom)).toBe(true);
    expect(isOpenOrderStatus('1', custom)).toBe(false); // '1' không nằm trong tập tùy biến
  });
});
